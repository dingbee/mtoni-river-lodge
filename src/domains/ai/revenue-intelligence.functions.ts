...
export const getRevenueIntelligenceOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const totalUnits = (rooms ?? []).reduce((s: any, r: any) => s + Number(r.total_units ?? 0), 0);
    ...
    const forwardRevenue = (forward ?? []).reduce((s: any, b: any) => s + Number(b.total ?? 0), 0);
    const forwardNights = (forward ?? []).reduce((s: any, b: any) => s + Number(b.nights ?? 0), 0);
    ...
    const past30Total = (past30 ?? []).reduce((s: any, b: any) => s + Number(b.total ?? 0), 0);
    const prev30Total = (prev30 ?? []).reduce((s: any, b: any) => s + Number(b.total ?? 0), 0);
    ...
    const atRisk = (forward ?? []).filter((b: any) => b.status === "pending").reduce((s: any, b: any) => s + Number(b.total ?? 0), 0);
...
export const generateRevenueForecast = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const confirmedRevenue = (forward ?? []).filter((b: any) => b.status !== "pending").reduce((s: any, b: any) => s + Number(b.total ?? 0), 0);
    const pendingRevenue = (forward ?? []).filter((b: any) => b.status === "pending").reduce((s: any, b: any) => s + Number(b.total ?? 0), 0);
    const confirmedNights = (forward ?? []).filter((b: any) => b.status !== "pending").reduce((s: any, b: any) => s + Number(b.nights ?? 0), 0);

    // Historical revenue per day, same season window
    const histRevPerDay = (history?.length ?? 0) > 0
      ? (history ?? []).reduce((s: any, b: any) => s + Number(b.total ?? 0), 0) / 365
      : 0;
...
export const getBookingPatterns = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const leadTimes = all.map((b: any) => Math.max(0, diffDays(b.created_at?.slice(0,10) ?? b.check_in, b.check_in)));
    ...
    const cancelled = all.filter((b: any) => b.status === "cancelled").length;
...
export const scanRevenueAlerts = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const soldNights = (forward ?? []).reduce((s: any, b: any) => s + Number(b.nights ?? 0), 0);
