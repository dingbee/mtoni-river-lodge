...
export function useHousekeepingRealtime(enabled=true){
 ...
 .on("postgres_changes",{event:"*",schema:"public",table:"housekeeping_events"},p=>{const x:any=p.new;if(p.eventType==="INSERT"&&x?.event_type)eventBus.emit({id:x.id,at:x.created_at,type:({task_created:"task.created",task_assigned:"task.assigned",cleaning_started:"task.updated",cleaning_completed:"task.completed",inspection_requested:"task.updated",inspection_passed:"task.updated",inspection_failed:"ops.alert_raised",exception_reported:"ops.alert_raised",exception_resolved:"ops.alert_resolved",maintenance_escalated:"ops.alert_raised",room_ready:"room.state_changed"} as any)[x.event_type]??"task.updated",userId:x.actor_id,module:"operations.housekeeping",entityType:"housekeeping",entityId:x.task_id??x.room_state_id??null,meta:{...x.payload,event_type:x.event_type},severity:x.event_type.includes("failed")||x.event_type.includes("maintenance")?"warn":"audit",correlationId:x.correlation_id});refresh();})
