...
function AdminBookings() {
  ...
  const bookings = useQuery({
    ...
    queryFn: () =>
      listFn({ data: { status, paymentStatus, from: from || undefined, to: to || undefined } }),
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        title="Filters"
        description="Narrow the reservation book by status, payment and arrival window."
      >
      ...
      <SectionCard className="p-0">
        <div className="overflow-x-auto">
          ...
          {bookings.error && (
            <div className="p-6 text-sm text-destructive">{(bookings.error as Error).message}</div>
          )}
              ...
              <tbody>
                {bookings.data.map((b: any) => (
                  <tr key={b.id} className="border-t border-border hover:bg-muted/40">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      {fmt(Number(b.total), b.currency)}
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex rounded-full px-2 py-0.5 text-[0.65rem] uppercase tracking-[0.15em] ${statusColors[b.status] ?? ""}`}
                      >
                        {b.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex rounded-full px-2 py-0.5 text-[0.65rem] uppercase tracking-[0.15em] ${paymentColors[b.payment_status] ?? ""}`}
                      >
              ...
              <>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-mono text-xs text-muted-foreground">
                      {detail.data.booking.reference}
                    </p>
                    <h2 className="font-display text-2xl">{detail.data.booking.guest_name}</h2>
                    <p className="text-sm text-muted-foreground">
                      {detail.data.booking.guest_email} · {detail.data.booking.guest_phone ?? "—"}
                    </p>
                  </div>
                  <button
                    onClick={() => setSelected(null)}
                ...
                <div className="mt-5 grid gap-2 rounded-lg border border-border bg-muted/30 p-4 text-sm sm:grid-cols-2">
                  <div>
                    ...
                    {detail.data.room?.name}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Status: </span>
                    <span
                      className={`rounded-full px-2 py-0.5 text-[0.65rem] uppercase ${statusColors[detail.data.booking.status]}`}
                    >
                      {detail.data.booking.status}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Check-in: </span>
                    {detail.data.booking.check_in}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Check-out: </span>
                    {detail.data.booking.check_out}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Adults: </span>
                    {detail.data.booking.adults}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Children: </span>
                    {detail.data.booking.children}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Country: </span>
                    {detail.data.booking.country ?? "—"}
                  </div>
                  <div>
                  ...
                  {detail.data.booking.pesapal_order_tracking_id && (
                    <div className="sm:col-span-2">
                      <span className="text-muted-foreground">Pesapal ref: </span>
                  ...
                  {detail.data.booking.payment_completed_at && (
                    <div>
                      ...
                      {new Date(detail.data.booking.payment_completed_at).toLocaleString()}
                    </div>
                  )}
                </div>
                ...
                <div className="mt-5">
                  ...
                  <ul className="mt-2 divide-y divide-border rounded-lg border border-border text-sm">
                    {detail.data.nights.map((n: any) => (
                      <li key={n.date} className="flex justify-between px-3 py-2">
                        <span>{n.date}</span>
                        <span>{fmt(n.nightly_rate, detail.data.booking.currency)}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                    ...
                    <ul className="mt-2 divide-y divide-border rounded-lg border border-border text-sm">
                      {detail.data.extras.map((e: any, i: number) => (
                        <li key={i} className="flex justify-between px-3 py-2">
                          ...
                          <span>{fmt(e.line_total, detail.data.booking.currency)}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="mt-5 grid grid-cols-2 gap-2 rounded-lg bg-muted/30 p-4 text-sm">
                  ...
                  <span className="text-right">
                    {fmt(Number(detail.data.booking.subtotal), detail.data.booking.currency)}
                  </span>
                  <span className="text-muted-foreground">Extras</span>
                  <span className="text-right">
                    {fmt(Number(detail.data.booking.extras_total), detail.data.booking.currency)}
                  </span>
                  <span className="font-display text-base">Total</span>
                  <span className="text-right font-display text-base">
                    {fmt(Number(detail.data.booking.total), detail.data.booking.currency)}
                  </span>
                  <span className="text-muted-foreground">Deposit (50%)</span>
                  <span className="text-right">
                    {fmt(
                      Number(
                        detail.data.booking.deposit_amount ??
                          Number(detail.data.booking.total) * 0.5,
                      ),
                      detail.data.booking.currency,
                    )}
                  </span>
                  <span className="text-muted-foreground">Balance (at check-in)</span>
                  <span className="text-right">
                    {fmt(
                      Number(
                        detail.data.booking.balance_amount ??
                          Number(detail.data.booking.total) * 0.5,
                      ),
                      detail.data.booking.currency,
                    )}
                ...
                <div className="mt-6">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {(["pending", "confirmed", "cancelled", "completed", "no_show"] as const).map(
                      (s) => (
                        <button
                          key={s}
