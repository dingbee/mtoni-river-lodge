...
function KitchenPage() {
  ...
  const breaching = rows.filter((t) => t.breaching);
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <StatCard
          ...
          value={
            (perf.data ?? []).length > 0
              ? `${(
                  (perf.data ?? []).reduce((s, p) => s + p.avg_prep_minutes, 0) / (perf.data ?? []).length
                ).toFixed(1)} min`
              : "—"
          }
          icon={Timer}
              ...
              <li key={t.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t.items.map((i) => `${Number(i.quantity)}× ${i.description}`).join(", ") || "No items"} ·{" "}
