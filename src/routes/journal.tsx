...
const posts = [
  ...
  { date:"November 2025", read:"3 min", title:"Wild ginger, baobab, rosehip", excerpt:"A short note on the East African botanicals at the heart of our spa.", img:spa },
];
...
export const Route = createFileRoute("/journal")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Stories from the riverbank — slow essays on land, season, and the small ceremonies of life at Mtoni." },
    ...
    <div className="bg-ivory text-charcoal">
      ...
      <section className="px-6 pt-40 pb-20 lg:px-12">
        <div className="mx-auto max-w-[1300px]">
          <Reveal><p className="eyebrow">Journal</p></Reveal>
          <Reveal delay={120}><h1 className="mt-6 max-w-4xl font-display text-5xl leading-[1.05] lg:text-7xl">Stories from the riverbank.</h1></Reveal>
          <Reveal delay={240}><p className="mt-8 max-w-xl text-charcoal/70">Slow essays on land, season, and the small ceremonies of life at Mtoni.</p></Reveal>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1300px]">
          ...
          <div className="mt-32 grid gap-x-10 gap-y-20 md:grid-cols-2">
            {posts.slice(1).map((p, i)=>(
              <Reveal key={p.title} delay={(i%2)*120} className={i%2 ? "md:mt-20" : ""}>
                <article className="group">
                  <div className="aspect-[4/5] overflow-hidden">
                    <img src={p.img} alt={p.title} className="h-full w-full object-cover transition-transform duration-[1500ms] group-hover:scale-105" loading="lazy" />
                  </div>
                  <p className="mt-6 text-xs uppercase tracking-[0.22em] text-muted-foreground">{p.date} · {p.read}</p>
                  <h3 className="mt-3 font-display text-2xl leading-snug">{p.title}</h3>
                  <p className="mt-3 max-w-md text-charcoal/70">{p.excerpt}</p>
                </article>
              </Reveal>
            ))}
          </div>
