...
function BookPage() {
  ...
  const [roomInventory, setRoomInventory] = useState<{ available_units: number; total_units: number } | null>(null);

  useEffect(() => {
    const slug = selectedRoom?.slug ?? incomingRoom;
    if (!slug) { setRoomInventory(null); return; }
    let cancelled = false;
