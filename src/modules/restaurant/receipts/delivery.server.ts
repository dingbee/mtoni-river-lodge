...
export function emailProviderConfigured(): boolean {
  // Lovable Emails is the configured provider for this project; the sender
  // domain is the switch. Kept as a function so the adapter can change.
  return process.env["EMAIL_SENDING_DISABLED"] !== "true";
}

export function whatsappProviderConfigured(): boolean {
  return Boolean(process.env["LOVABLE_API_KEY"] && process.env["TWILIO_API_KEY"] && process.env["WHATSAPP_FROM"]);
}
  ...
  if (method === "email") {
    ...
    try {
      ...
      const res = await sendTransactionalInternal({
        templateName: "restaurant-receipt",
        recipientEmail: to,
      ...
      if (res.ok) return { status: "sent", provider: "lovable_email", providerReference: res.messageId, recipient: to };
      return {
        status: "failed",
        provider: "lovable_email",
        recipient: to,
        failureCode: res.reason === "suppressed" ? "provider_rejected" : "provider_rejected",
        failureReason:
          res.reason === "suppressed"
            ? "This address previously unsubscribed or bounced, so it is suppressed."
            : (res.error ?? DELIVERY_FAILURE_MESSAGES.provider_rejected),
      };
    } catch (e: any) {
      return {
        status: "failed",
        ...
        failureReason: e?.message ?? DELIVERY_FAILURE_MESSAGES.network_timeout,
      };
    }
  }
  ...
  try {
    ...
    if (!res.ok) {
      return {
        ...
        failureReason: json?.message ?? `Provider responded ${res.status}.`,
      };
    }
    // Twilio confirms acceptance, not receipt — "sent", never "delivered".
    return { status: "sent", provider: "twilio_whatsapp", providerReference: json?.sid, recipient: phone };
  } catch (e: any) {
    return {
      status: "failed",
      provider: "twilio_whatsapp",
